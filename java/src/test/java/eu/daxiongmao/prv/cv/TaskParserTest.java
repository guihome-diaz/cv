...
class TaskParserTest {
    ...
    void experienceWithTasks() {
        ...
        ExperienceWithTasks futureCitizen = cv.experiences().stream().filter(experience -> "Office Administrator".equals(experience.jobTitle())).findAny().get();
        List<TaskParser.TaskItem> tasks = futureCitizen.tasks();
        Assertions.assertNotNull(tasks);
        Assertions.assertEquals(1, tasks.size());
        Assertions.assertEquals("Retaining all prior responsibilities, took on the following:", tasks.getFirst().text());
        Assertions.assertEquals(3, tasks.getFirst().subItems().size());
        Assertions.assertEquals("Managed social media platforms (Facebook, LinkedIn)", tasks.getFirst().subItems().getFirst());
        Assertions.assertEquals("Filed and updated RCS registrations", tasks.getFirst().subItems().get(1));
        Assertions.assertEquals("Managed company intellectual property registration and renewals (EUIPO, EPO)", tasks.getFirst().subItems().get(2));
    }
    ...
    void experienceWithSubTasks() {
        ...
        List<TaskParser.TaskItem> tasks = ceratizitCSA.tasks();
        Assertions.assertNotNull(tasks);
